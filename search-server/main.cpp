...
public:
	...
	int GetDocumentId(int index) const {
		if (index < 0) {
			throw out_of_range("smaller than zero");
		}
		else if (index >= GetDocumentCount()) {
			throw out_of_range("bigger than number of documents");
...
private:
	...
	QueryWord ParseQueryWord(string text) const {
		...
		if (text.empty()) {
			throw invalid_argument("space after mines");
		}else if (text[0] == '-') {
			throw invalid_argument("second mines");
		}
		else if (!IsValidWord(text)) {
			throw invalid_argument("invalid query");
		}

		QueryWord result = QueryWord{ text, is_minus, IsStopWord(text) };
		return result;
	}
